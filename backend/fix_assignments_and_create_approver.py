...
def fix_assignments_and_create_approver():
    ...
    try:
        ...
        for example in examples:
            _, created = Assignment.objects.get_or_create(project=project, example=example, assignee=bz_user)
            if created:
                count += 1
    ...
    try:
        ...
        member, created = Member.objects.get_or_create(project=project, user=sh_user, defaults={"role": role_approver})
        ...
        count_sh = 0
        for example in examples:
            _, created = Assignment.objects.get_or_create(project=project, example=example, assignee=sh_user)
            if created:
                count_sh += 1
